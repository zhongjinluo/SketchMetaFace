...
import cv2
from .DeepImplicitTemplates.networks.deep_implicit_template_decoder import Decoder
...
class HGPIFuNetwNML(BasePIFuNet):
    ...
    def __init__(self, 
        ...
        #     last_op=None)
        self.mlp = Decoder(latent_size=257)
    ...
    def query(self, points, calibs, transforms=None, labels=None, update_pred=True, update_phi=True):
        ...
        for i, im_feat in enumerate(self.im_feat_list):
            ...
            '''
            if self.is_train:
                point_local_feat_list = [self.index(im_feat, xy), sp_feat, points]       
                point_local_feat = torch.cat(point_local_feat_list, 1)
                #pred, phi = self.mlp(point_local_feat)
                point_local_feat= point_local_feat.permute(0, 2, 1).reshape(-1, 260)
                # print(point_local_feat.shape)
                pred = self.mlp(point_local_feat)
                pred= pred.reshape(2, 10000, 1).permute(0, 2, 1)
                # print(pred.shape)
                ...
                points_size = points.shape[2]
                point_local_feat_list = [self.index(im_feat, xy), sp_feat, points]       
                point_local_feat = torch.cat(point_local_feat_list, 1)
                point_local_feat= point_local_feat.permute(0, 2, 1).reshape(-1, 260)
                pred = self.mlp(point_local_feat)
                pred = pred.reshape(1, points_size, 1).permute(0, 2, 1)
                # print(pred.shape)
    ...
    def get_error(self, gamma):
        ...
        if self.nmltrain:
            ...
            for preds in self.intermediate_preds_list:
                ...
                labels = torch.clamp(self.labels, -1, 1)
