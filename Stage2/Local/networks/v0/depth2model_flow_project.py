...
from networks.v0.nd2hd.model import Generate as DepthGenerate
from networks.v0.dd2flow.model import Generate as FlowGenerate
...
class Depth2Model_flow_project:
    def __init__(self):
        ...
        self.generator = DepthGenerate(4, 1)
        model_CKPT = torch.load("./checkpoints/nd2hd/checkpoints/dicts/latest.pth")
        self.generator.load_state_dict(model_CKPT)
        self.generator.cuda().eval()

        self.flow_generator = FlowGenerate(2, 2)
        model_CKPT = torch.load("./checkpoints/dd2flow/checkpoints/dicts/latest.pth")
        self.flow_generator.load_state_dict(model_CKPT)
        self.flow_generator.cuda().eval()
    ...
    def predict_from_coarse_model(self, S_list, vertices, faces, out_dir=None, name=None):
        ...
        for di in range(1):
            ...
            # cv2.imwrite(os.path.join(out_dir.replace("HD2", "HD1"), name+".png"), CD_img)
            # depth2mesh(CD_img, out_dir, name+"_c_a_"+str(di+1))
        end = time.time()
        ...
        with torch.no_grad():
            ...
            RD_tensor = self.generator(SCD.cuda())
            DD = torch.cat([RD_tensor, CD_tensor.cuda()], axis=1)
            FLOW = self.flow_generator(DD)
            end = time.time()
            print("depth_predict:", end-start)

            depth_preds = (RD_tensor.permute(0,2,3,1) + 1) / 2 * 255.0
            depth_preds = depth_preds.cpu().detach().numpy()
            FLOW = (FLOW.permute(0,2,3,1) + 1) / 2
            FLOW = FLOW.cpu().detach().numpy()
            FLOW = FLOW * 100.0 - 50.0
            flow = FLOW[0,:,:,:]
            Tns = []
            for di in range(5):
                RD = np.array(depth_preds[di,:,:,:] * 255.0, dtype=np.uint16).reshape(512, 512)
                cv2.imwrite(os.path.join(out_dir, name+".png"), RD)

                warp = cv_warp(RD, flow)
                # cv2.imwrite("test.png", warp)
                warp_out = np.ones(warp.shape) * 255.0 * 255.0
                warp_out[mask] = warp[mask]
                warp_out = warp_out.astype(np.uint16)
                # cv2.imwrite(os.path.join(out_dir.replace("HD2", "WHD2"), name+".png"), warp_out)
                RD = warp_out
        ...
        for i in range(1):
            ...
            front_indices = get_indices(V, sigma=1e-3)
            points = torch.from_numpy(V[front_indices].T).float()
            pts = project(points.unsqueeze(0), self.calib.unsqueeze(0))
            xy = pts[:, :2, :]
            XY = (xy[0] + 1) * 512 * 0.5
            ...
            Z = RD[XY[1], XY[0]] / 255.0 / 255.0
            boundary_indices = np.where(Z > 0.98)[0]
            Z[boundary_indices] = (np.array(V[front_indices, 2][boundary_indices]) + 1) * 0.5
            
            DELTA_Z = np.array(V[front_indices, 2] + 1) * 0.5 - np.array(Z)
            mean_value = DELTA_Z.mean()
            DELTA_Z = DELTA_Z - mean_value

            filter_indices = np.where(DELTA_Z < -0.05)[0]
            Z[filter_indices] = (np.array(V[front_indices, 2][filter_indices]) + 1) * 0.5

            filter_indices = np.where(DELTA_Z > 0.05)[0]
            Z[filter_indices] = (np.array(V[front_indices, 2][filter_indices]) + 1) * 0.5

            vertices = np.array(V[front_indices])
            vertices[:, 2] = Z
            vertices[:, 2] = vertices[:, 2] * 2 - 1

            V[front_indices] = vertices

            T0.vertices = A.dot(V)

        T0.export(os.path.join(out_dir.replace("HD2", "M2_warp_project"), name+".obj"))
        end = time.time()
...
if __name__ == '__main__':
    d2m = Depth2Model()
    root = "/program/SIGRAPH22/ALGO_V6/check_problem2/3dviewer/data/sketch"
    for f in os.listdir(root):
        S_list = []
        S = Image.open(os.path.join(root, f))
        S_list.append(S)
        ...
        if os.path.exists(obj_path):
            ...
            RD = d2m.predict_from_coarse_model(S_list, CM.vertices, CM.faces, "/program/SIGRAPH22/ALGO_V6/check_problem2/3dviewer/data/output_wflow5_plus2/", f.replace(".png", ""))
            print(f)