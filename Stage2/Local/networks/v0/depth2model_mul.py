...
from networks.v0.multi_camera.render.camera import Camera
from networks.v0.multi_camera.render.gl.color_render import ColorRender
from networks.v0.multi_camera.model import Generate

# from multi_camera.render.camera import Camera
# from multi_camera.render.gl.color_render import ColorRender
# from multi_camera.model import Generate
...
class Depth2Model:
    def __init__(self):
        ...
        model_CKPT = torch.load("./networks/v0/multi_camera/checkpoints/dicts/latest.pth")
    ...
    def predict_from_coarse_model(self, S_list, vertices, faces, out_dir=None, name=None):
        ...
        with torch.no_grad():
            ...
            # CD_tensor = torch.stack(CD_tensor_list).float()
            ...
            for di in range(5):
                ...
                if di == 0:
                    ...
                    Tns.append(V)
                    # print(verts.shape)
                # m = om.TriMesh(points=V)
                # om.write_mesh("outputs/" + name + "_a_"+str(di+1)+"_point.obj", m)
                break
        end = time.time()
        ...
        # T0.export(os.path.join(out_dir, name+"_c.obj"))
        T0.vertices = normalize(T0.vertices)
        ...
        for j in [1, 2, 3, 4, 5]:
            ...
            if j == 1:
                ...
                front_indices = get_indices_plus(V)
                points = torch.from_numpy(V[front_indices].T).float()
                pts = project(points.unsqueeze(0), self.calib.unsqueeze(0))
                xy = pts[:, :2, :]
                XY = (xy[0] + 1) * 512 * 0.5
                ...
                Z = TND[XY[1], XY[0]] / 255.0 / 255.0
                V[front_indices, 2] = Z * 2 - 1
                Tns.append(np.dot(self.Ms_re[j-1], V[front_indices].T).T)
            break
...
if __name__ == '__main__':
    d2m = Depth2Model()
    root = "/program/SIGRAPH22/enhancement2/data/Sn/"
    data_dir = "/program/SIGRAPH22/enhancement2/check/3dviewer/data/deform/"
    for f in os.listdir(data_dir):
        # if ("_a_1.png" not in f) or ("RAW_" not in f):
            # continue
        # f = "RAW_mesh_Zooey+Deschanel_C00022_a_1.png"

        if "RAW_" not in f:
            continue
        f = f.replace("_5.obj", "_a_1.png")

        print(f)
        S_list = []
        for di in range(0, 5):
            S = Image.open(os.path.join(root, f.replace("_a_1.png", "_a_"+str(di+1)+".png")))
            S_list.append(S)
        CM = trimesh.load(os.path.join("/program/SIGRAPH22/enhancement2/T0_front/", f.replace("_a_1.png", ".obj")), process=False)
        RD = d2m.predict_from_coarse_model(S_list, CM.vertices, CM.faces, "outputs/", f.replace("_a_1.png", ""))
        # break
    # root = "/program/SIGRAPH22/APP_V3/SimpModeling_Depth_DIT_RENDER3/networks/v0/nd2rd/training_dataset/D/"
    # for f in os.listdir(root):
    #     if f == "0.png":
    #         continue
    #     N = np.array(Image.open(os.path.join("/program/SIGRAPH22/APP_V3/SimpModeling_Depth_DIT_RENDER3/networks/v0/nd2rd/training_dataset/N/", f)).convert('RGB'))
        
    #     # CD = cv2.imread(os.path.join("/program/SIGRAPH22/APP_V3/SimpModeling_Depth_DIT_RENDER3/networks/v0/nd2rd/training_dataset/D/", f), -1)
    #     # CD = np.array(CD/255.0/255.0).reshape(512, 512, 1)
    #     CM = trimesh.load(os.path.join("/program/SIGRAPH22/APP_V3/SimpModeling_Depth_DIT_RENDER3/gallery/coarse/", f.replace(".png", ".obj")), process=False)
        
    #     RD = d2m.predict_from_coarse_model(N, CM.vertices, CM.faces, "/program/SIGRAPH22/APP_V3/SimpModeling_Depth_DIT_RENDER3/networks/v0/nd2rd/training_dataset/outputs/", f.replace(".png", ""))
    #     cv2.imwrite('/program/SIGRAPH22/APP_V3/SimpModeling_Depth_DIT_RENDER3/networks/v0/nd2rd/training_dataset/outputs/'+f, RD)
    #     print(f)
        # break