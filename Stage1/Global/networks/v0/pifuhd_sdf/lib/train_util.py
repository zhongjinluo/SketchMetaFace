...
def gen_mesh_server2(opt, net, cuda, data, save_path, use_octree=True):
...
def gen_mesh_part(opt, net, cuda, data, save_path, use_octree=True):
    ...
    try:
        ...
        verts, faces = reconstruction_part(
            net, cuda, calib_tensor, opt.resolution, b_min, b_max, use_octree=use_octree)
        # save_obj_mesh(save_path, verts, faces)
        mesh = trimesh.Trimesh(vertices=verts, faces=faces, process=False)
        mesh.export(save_path)
