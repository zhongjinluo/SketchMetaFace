...
def gen_landmark(opt, net, cuda, data, save_path, use_octree=True):
    ...
    Image.fromarray(np.uint8(save_img[:,:,:3])).save(save_img_path)
    ind, sdf = reconstructionLM(
        net, cuda, calib_tensor, target, opt.resolution, b_min, b_max, use_octree=use_octree)
    print('=== sdf: ', sdf.shape)
