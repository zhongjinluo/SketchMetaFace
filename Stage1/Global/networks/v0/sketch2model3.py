...
class Sketch2Model3:
    def __init__(self, projection_mode='orthogonal'):
        ...
        # 3 modified
        netG.load_state_dict(torch.load(os.path.join("./checkpoints/pifu_occ_main/example", "netG_latest"), map_location=cuda))
    ...
    def predict_part(self, norm, save_path=os.path.join(os.path.abspath(os.path.dirname(__file__)), "m.obj")):
        norm = self.to_tensor(Image.fromarray(norm).convert('RGB'))
        render = torch.cat([norm], 0).unsqueeze(0)
        data = {}
        with torch.no_grad():
            ...
            data['b_max'] = np.array([1, 1, 1])
            data['calib'] = self.calib.unsqueeze(0)
            return gen_mesh_part(self.opt, self.netG, self.cuda, data, save_path, use_octree=False)
